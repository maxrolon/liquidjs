  ...
  liquid.registerTag('section', {
    ...
    render: function (scope, hash) {
      ...
      return liquid.getTemplate(filepath, scope.opts.root)
        .then((templates) => {
          scope.push(hash)
          return liquid.renderer.renderTemplates(templates, getScope(filepath, scope))
        })
        .then((html) => {
          scope.pop()
...
const getScope = (filepath, scope) => {
  const moduleDir = scope.opts.root.find(dir => ~dir.indexOf('/modules'))
  const mockPath = path.resolve(moduleDir, `${filepath}/mock`)
  const mockFile = require(mockPath)
  scope.push(mockFile)
  return scope
}